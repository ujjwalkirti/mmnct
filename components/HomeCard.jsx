...
const HomeCard = () => {
    ...
    const setCurrOver = () => {
      if (team[0]) {
        if (team[0].currBattingTeam === team[0].Team1Id) {
          const val = getOver(team[0].Team1Score, team[0].Team1prev, team[0].Team1Extra)[1];
          const data = val.split(' ');
          data.pop();
          setover_summary(data);
        } else {
          const val = getOver(team[0].Team2Score, team[0].Team2prev, team[0].Team2Extra)[1];
          const data = val.split(' ');
          data.pop();
          setover_summary(data)
        }
  ...
  const shortformstyle = " rounded-full h-[52px] aspect-square w-[52px] md:h-[50px] md:w-[62px]";
  useEffect(() => {
    getdata();
  }, []);
            ...
            <div className=" flex justify-between text-black items-center">
              <p className="  text-2xl font-extrabold text-[#F8C156] px-2 bg-white shadow-lg rounded-2xl">
                ...
                <span className=" text-red-600 mx-1 rounded-full">
                  <VscCircleFilled className=" animate-pulse inline-block  text-sm bg-orange-600 overflow-hidden rounded-full t" />
                </span>
                Live
              </p>
          ...
          <div className=" teams flex px-2 mb-1">
            <div className=" team_1 flex justify-around  w-[45%]">
              <div className=" flex justify-center text-center  flex-col  ">
                ...
                {/* Team 1 name info */}
                <p className=" text-black text-sm font-bold">{teams[team[0].Team1Id].teamCode}</p>
                <p className="text-[10px] text-gray-500">({teams[team[0].Team1Id].teamType})</p>
              </div>
                ...
                <p className=" text-sm font-bold">
                  {totalScore(
                    ...
                    <>
                      ...
                      <span className=" text-xs font-thin">({totalScore(
                        team[0].Team1Score,
                        team[0].Team1Extra,
                        team[0].Team1Wicket
                      ) === "0/0" &&
            ...
            <div className=" w-[13%]  ">
              <Image src="/vector-8.png" height={60} width={60} />
            </div>
            <div className=" team_2 flex flex-row-reverse justify-around  w-[45%]">
              <div className=" flex justify-center text-center  flex-col  ">
                ...
                {/* team 2 name info */}
                <p className=" text-black text-sm font-bold">{teams[team[0].Team2Id].teamCode}</p>
                <p className=" text-[10px] text-gray-500">({teams[team[0].Team2Id].teamType})</p>
              </div>
              <div className=" score  text-red-600 translate-y-4">
                <p className=" text-sm font-bold">
                  {totalScore(
              ...
              team[0].currBattingTeam === team[0].Team1Id ?
                <>
                  <div className=" Bat_stat text-left  ">
                    ...
                    <p className=" text-sm font-bold mb-1">{teams[team[0].Team1Id].teamCode} Batting</p>
                    <p className=" text-xs font-light">{team[0].Team1Players[team[0].striker]?.playerName.split(' ')[0]}:  {getPlayerScore(team[0]?.Team1Players, team[0]?.striker)}</p>
                    <p className="text-xs font-light">{team[0].Team1Players[team[0].nonStriker]?.playerName.split(' ')[0]}: {getPlayerScore(team[0]?.Team1Players, team[0]?.nonStriker)}</p>
                  </div>
                  <div className=" Bowl_stat text-right">
                    {/* Active Bowler info */}
                    <p className=" text-sm font-bold">{teams[team[0].Team2Id].teamCode}-Bowling</p>
                    <p className=" text-xs font-light">{team[0].Team2Players[team[0].baller]?.playerName.split(' ')[0]}: {team[0].Team2Players[team[0].baller]?.score[14]}-{team[0].Team2Players[team[0].baller]?.score[13]}({Math.floor(team[0].Team2Players[team[0].baller]?.score[12] / 6)}.{team[0].Team2Players[team[0].baller]?.score[12] % 6})</p>
                  </div></>
                :
                <>
                  ...
                  <div className=" Bowl_stat text-left">
                    {/* Active Bowler info */}
                    <p className=" text-sm font-bold">{team[0]?.Team1Id.teamCode} Bowling</p>
                    <p className=" text-xs font-light">{team[0]?.Team1Players[team[0]?.baller]?.playerName.split(' ')[0]}: {team[0]?.Team1Players[team[0].baller]?.score[14]}-{team[0]?.Team1Players[team[0].baller]?.score[13]}({Math.floor(team[0]?.Team1Players[team[0].baller]?.score[12] / 6)}.{team[0]?.Team1Players[team[0]?.baller]?.score[12] % 6})</p>
                  </div>
                  <div className=" Bat_stat text-right  ">
                    {/* batting team stats -- striker and non-striker */}
                    <p className=" text-sm font-bold mb-1">{teams[team[0].Team2Id].teamCode} Batting</p>
                    <p className=" text-xs font-light">{team[0]?.Team2Players[team[0].striker]?.playerName.split(' ')[0]}: {getPlayerScore(team[0]?.Team2Players, team[0]?.striker)}</p>
                    <p className="text-xs font-light">{team[0]?.Team2Players[team[0].nonStriker]?.playerName.split(' ')[0]}: {getPlayerScore(team[0]?.Team2Players, team[0]?.nonStriker)}</p>
                  </div>
                </>
            }
          </div>
